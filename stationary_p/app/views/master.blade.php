...
<body>
	<script type="text/javascript">
		...
		var gCounter=0;
		function CheckboxCol(id,val){
			
			text=val;
			var Icon='<span style="font-size:17px;">&nbsp '+text+'&nbsp</span>';
			gCounter++;
			chk='<label class="btn btn-block btn-warning" for="check_'+id+'_'+gCounter+'" style="height:100%;width:100%;"><input class="approvals" type="checkbox" name="approvals" id="check_'+id+'_'+gCounter+'" value="'+val+'"></input>'+
			Icon+'</label>';
			return chk;
		}
		...
		function selectAllButton(text,uniq,checked){
			return "<button class='btn btn-default btn-m' type='button' onclick='selectAll(\""+uniq+"\",\""+checked+"\")'>"+text+"</button>";
		}
		function selectAll(uniq,checked){
			//x=document.getElementsByTagName('input');
			T_F = parseInt(checked)==1;//?true:false;
			x=$('input[type=checkbox]'); 
			for (var i = 0;i<x.length; i++) {
				if(x[i].id.indexOf(uniq)>-1)
					x[i].checked=T_F;
				console.log(T_F);
			}
		}
		...
		function insertSelTable(checkB,cellObjData,divID,role_id,table_fields,table_fields2){
			...
			zeroRow.insertCell().innerHTML=selectAllButton('Select All',divID+'_',1);
			zeroRow.insertCell().innerHTML=selectAllButton('Deselect All',divID+'_',0);
			...
			for(var i=0;i<table_fields2.length;i++)
				...
				cellObjData.forEach(function(row,counter){
					...
					if(checkB==1)current_row.insertCell().innerHTML=CheckboxCol(divID+'_'+row['id'],row['id']);
		...
		function insertVertTable(checkB,cellObjData,divID,role_id,table_fields,table_fields2){
			...
			cellObjData.forEach(function(row,counter){//this should be 1
				table_fields.forEach(function(col){
					var current_row=table1.insertRow();
					current_row.insertCell().innerHTML=col;//CheckboxCol(divID+'_'+row['id'],row['id']);
			...
			function insertStudTable(checkB,cellObjData,divID,role_id,table_fields,table_fields2){
				...
				cellObjData.forEach(function(row,counter){//this should be 1
					var current_row=table1.insertRow();
					if(checkB==1)current_row.insertCell().innerHTML=CheckboxCol(divID+row['id'],row['id']);
					table_fields.forEach(function(col){
